...
The library uses `msgspec`_ for serialisation, which respects the Python type hints:
...
.. code-block:: python
   ...
   from cachetout import Cache, cache

   cache = Cache("temp_cache")

   # Expire in 1 hour
   cache.set(
       "temp_data", "value", expires_at=datetime.now(tz=UTC) + timedelta(hours=1)
   )
